...
async def fetch_details_parallel(video_id):
    async with aiohttp.ClientSession() as session:
        tasks = [asyncio.create_task(fetch_from_instance(session, video_id, url)) for url in INVIDIOUS_INSTANCES]
...
async def process_video_async(video_id, metadata_override=None):
    ...
    async def step_piped():
        return await find_piped(video_id)
